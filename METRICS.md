...
Typical downstream pattern:
    ...
    nmd_near    <- read.delim("runs/<dataset>/metrics/nmd_fragility_nearest.tsv", na = "NA")
    nmd_any     <- read.delim("runs/<dataset>/metrics/nmd_fragility_any.tsv", na = "NA")
    nmd_window  <- read.delim("runs/<dataset>/metrics/nmd_fragility_distal_window.tsv", na = "NA")
    ...
    merged <- manifest %>%
      ...
      left_join(nmd_near,   by = c("transcript_id", "gene_id"),
                suffix = c("", "_nearest")) %>%
      left_join(nmd_any,    by = c("transcript_id", "gene_id"),
                suffix = c("", "_any")) %>%
...
explicit `nmd_model` column. If multiple models are enabled, prefer 
loading them as separate tables and joining with suffixes 
(e.g., `_nearest`, `_any`, `_window`) rather than concatenating without 
...
are implicitly applying the any-EJC model to the actual stop codon, 
so `nmd_fragility_any` is the natural companion.